...
const Playground: React.FC = () => {
  ...
  // Handler for seat click events
  const handleSeatClick = (action: string, seat: SeatData) => {
    console.log("Seat clicked:", action, seat);
  };
      ...
      {!layout ? (
        <div>
          <div
            role="button"
            tabIndex={0}
            className="mx-auto my-10 flex h-[12rem] max-w-3xl cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-4 text-center"
            onClick={handleClick}
            onKeyDown={(e) => {
          ...
          >
            ...
            <Upload className="size-[4rem] mt-8 text-gray-500/25" />
          </div>
        </div>
      ) : (
        <SeatPicker layout={layout} 
        onSeatAction={handleSeatClick} readOnly />
      )}
      {/* Tab Switcher */}
